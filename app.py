...
# Render: ключ берётся только из переменной окружения UI_OPENAI_KEY
...
def diarize_by_llm(client: OpenAI, raw_transcript: str) -> str:
    ...
    logging.warning(
        "LLM diarization failed, using naive alternation fallback. Reason: %s",
        last_err
    )
    sents = [
        s.strip()
...
async def analyze(request: Request):
    ...
    if client is None:
        ...
        return JSONResponse(
            ...
            content={
                "status": "error",
                "message": "Не удалось инициализировать OpenAI-клиент. Проверьте ключ в UI_OPENAI_KEY.",
            },
        )
    ...
    try:
        if "application/json" in content_type:
            ...
            criteria = normalize_criteria(
                data.get("criteria") if isinstance(data, dict) else None
            )
        else:
